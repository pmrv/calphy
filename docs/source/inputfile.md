...
```{note}
**Finite pressure.** During the sweep the scaled system has to be barostatted at **λp** (sampling λU at (T₀, λp) is the real system at (T₀/λ, p)). LAMMPS barostats can only ramp their target linearly over a run and accept no variable target, so λ must be linear in the MD step for the barostat to track λp exactly. `linear` is; `uniform_temperature` is a hyperbola in the step. calphy therefore runs a `uniform_temperature` sweep as **piecewise-linear segments**: the sweep is split into up to 20 sub-runs whose end points lie on the exact schedule, λ follows `ramp()` within each, and the barostat chord p·λ equals λp at every step. The trade-off is where the approximation lands. The *pressure and the free-energy integral are exact*, since the integral runs over the recorded λ grid. What deviates from the ideal schedule is only the *sampling density*, by O(1/K²): about 0.2 % at K = 20 for a 1000 → 3000 K sweep, i.e. some Kelvin bins get 0.2 % more samples than others. Every segment boundary re-issues the integrator fix, which resets the barostat velocity, so segments are kept at least ten damping times long; very short sweeps (a few thousand steps) fall back to fewer segments, down to a single linear ramp, which then *is* the `linear` schedule.
```
